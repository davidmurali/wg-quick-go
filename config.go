...
type Config struct {
	...
	// AirgapPerformanceOptimization - optimization will happen only if it is "Yes" otherwise its regular flow
	AirgapPerformanceOptimization string
...
{{- if .AirgapPerformanceOptimization }}{{ "\n" }}AirgapPerformanceOptimization = {{ .AirgapPerformanceOptimization }}{{ end }}
{{- range .Peers }}
{{- "\n" }}
[Peer]
...
func parseInterfaceLine(cfg *Config, lhs string, rhs string) error {
	...
	case "PrivateKey":
		...
		cfg.PrivateKey = &key
	case "AirgapPerformanceOptimization":
		cfg.AirgapPerformanceOptimization = strings.ToLower(rhs)
	default:
